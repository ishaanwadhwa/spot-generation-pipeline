...
export const BLINDS = { SB: 0.5, BB: 1.0 } as const;

type Street = "p" | "f" | "t" | "r";
type SizeRef = number | "pot";

type HistoryAction =
  | ["-", Street]
  | [string, "x" | "f"]
  | [string, "c", null, number]
  | [string, "b", SizeRef, number]
  | [string, "r", string, number]
  | [string, "a", "AI" | null, number];
...
export interface SpotOutputLike {
  id: string;
  fmt: string;
  str: Street;
  difficulty: number;
  ...
  data: {
    ...
    fmt: string;
    str: Street;
    hero: { pos: string; hand: [string, string] };
    v: string[];
    brd: string[];
    pot: number;
    hist: HistoryAction[];
    opts: Option[];
    sol: { b: number; ev: number[] };
    meta?: {
      concept: string[];
      summary?: string;
      solverNotes?: string[];
...
function isStreetMarker(a: unknown): a is ["-", Street] {
  return (
    Array.isArray(a) &&
    a.length === 2 &&
    a[0] === "-" &&
...
function sumActionContrib(action: unknown): number {
  if (!Array.isArray(action) || action.length < 2) return 0;
  const code = action[1];
...
export function potFromHist(hist: unknown[]): number {
  let pot = BLINDS.SB + BLINDS.BB;
  for (const a of hist) pot += sumActionContrib(a);
  return pot;
}

function validateStreetOrder(hist: unknown[], errors: string[]) {
  const order: Street[] = ["p", "f", "t", "r"];
  let curIdx = -1;
  for (const a of hist) {
    if (!isStreetMarker(a)) continue;
...
function validateActionTuple(a: unknown, errors: string[], ctx: string) {
  if (!Array.isArray(a)) {
    errors.push(`Non-array ${ctx} action: ${JSON.stringify(a)}`);
    return;
  }
  if (isStreetMarker(a)) return;
...
function validateOptionTuple(opt: unknown, errors: string[]) {
...
function validateSizingAgainstPot(spot: SpotOutputLike, errors: string[]) {
  // Check only % sizing (numeric) -> exact ~= (pct/100)*pot at decision point
  const pot = spot.data.pot;
  if (!isFiniteNumber(pot) || pot <= 0) return;

  for (const opt of spot.data.opts || []) {
    if (!Array.isArray(opt) || opt[0] !== "b") continue;
    const sizeRef = opt[1];
    const exact = opt[2];
    if (!isFiniteNumber(sizeRef)) continue;
...
function validateMeta(spot: SpotOutputLike, errors: string[]) {
...
function validateIntentCompatibility(spot: SpotOutputLike, errors: string[]) {
  ...
  try {
    ...
    if (!Array.isArray(brd) || brd.length < 3) return;
...
export function validateSpotOutput(spot: unknown): { ok: boolean; errors: string[] } {
  const errors: string[] = [];
  const s = spot as SpotOutputLike;
  ...
  if (Array.isArray(data.hist)) {
    ...
    for (const a of data.hist) validateActionTuple(a, errors, "hist");
  }
  if (Array.isArray(data.opts)) {
    for (const o of data.opts) validateOptionTuple(o, errors);
  }
