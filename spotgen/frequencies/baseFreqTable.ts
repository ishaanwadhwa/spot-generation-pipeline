...
// Note: HandIntent now includes "give_up" but those hands are filtered before reaching frequency lookup
import type { HandIntent } from "../poker/intent";
import type { TurnType } from "../poker/turnClassify";

type NodeIntentKey = "value" | "pressure" | "semi_bluff" | "bluffcatch";
...
export const BASE_FREQ_TABLE: Partial<Record<
  HandIntent,
  Partial<Record<TurnType, Partial<Record<NodeIntentKey, number[]>>>>
>> = {
  // ═══════════════════════════════════════════════════════════════════════════
  // 1️⃣ MADE VALUE
 ...
 * Missing keys are features, not bugs.
 */
function balancedFallback(n: number): number[] {
  if (n <= 0) return [];
  if (n === 1) return [1.0];
 ...
 */
function resizeFrequencies(source: number[], targetLength: number): number[] {
  if (source.length === targetLength) return [...source];
  if (targetLength <= 0) return [];
  if (targetLength === 1) return [1.0];
 ...
 */
export function lookupBaseFrequencies(
  ...
  if (numOptions === 1) return [1.0];

  const nodeKey = nodeIntent as NodeIntentKey;
  const handEntry = BASE_FREQ_TABLE[handIntent];
