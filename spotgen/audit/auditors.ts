import type { AuditPacket, RegenConstraints } from "./types";
import { checkBarrelEligibility } from "../poker/barrelEligibility";
import { valueOfRank, rankOf } from "../poker/ranks";

function isEmptyConstraints(c: RegenConstraints): boolean {
  return Object.keys(c).length === 0;
}

function mergeConstraints(base: RegenConstraints, next: RegenConstraints): RegenConstraints {
  const out: RegenConstraints = { ...base, ...next };
...
export interface Auditor {
  audit(packet: AuditPacket): RegenConstraints;
  merge(base: RegenConstraints, next: RegenConstraints): RegenConstraints;
}
 ...
 */
export class RuleBasedAuditor implements Auditor {
  merge(base: RegenConstraints, next: RegenConstraints): RegenConstraints {
  ...
  audit(packet: AuditPacket): RegenConstraints {
    ...
    const boardRanks = board.map((c: string) => valueOfRank(rankOf(c)));

    // Check barrel eligibility
    const eligibility = checkBarrelEligibility(heroClass, hand, turnType, boardRanks);

    // Get offered bet sizes
    const hasBigBarrelOpts = s.data.opts.some(
      (o: any) => Array.isArray(o) && o[0] === "b" && (o[1] === 75 || o[1] === 100)
    );
    const hasSmallBarrelOpts = s.data.opts.some(
      (o: any) => Array.isArray(o) && o[0] === "b" && (o[1] === 33 || o[1] === 50)
    );
    ...
    if (hand.hasPair && hand.pairRank && turnType === "overcard_turn") {
      const pairVal = valueOfRank(hand.pairRank);
