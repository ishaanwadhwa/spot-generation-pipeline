...
import { classifyPairQuality, type PairQuality } from "./pairQuality";
import type { TurnType } from "./turnClassify";

export type HeroHandClass = "monster" | "strong_value" | "medium" | "weak" | "air";

function countRanks(cards: string[]): Map<string, number> {
  const m = new Map<string, number>();
  for (const c of cards) m.set(rankOf(c), (m.get(rankOf(c)) || 0) + 1);
  return m;
}

function hasFlushMade(hero: [string, string], board: string[]): boolean {
  const suits = new Map<string, number>();
  for (const c of [...hero, ...board]) suits.set(suitOf(c), (suits.get(suitOf(c)) || 0) + 1);
  for (const n of suits.values()) if (n >= 5) return true;
  return false;
}

function hasStraightMade(hero: [string, string], board: string[]): boolean {
  const allRanks = new Set<number>([...hero, ...board].map((c) => valueOfRank(rankOf(c))));
  // Check for wheel
  if (allRanks.has(14) && allRanks.has(2) && allRanks.has(3) && allRanks.has(4) && allRanks.has(5)) return true;
  // Check for regular straights
  for (let hi = 14; hi >= 6; hi--) {
    let ok = true;
    for (let x = hi; x > hi - 5; x--) if (!allRanks.has(x)) ok = false;
    if (ok) return true;
  }
  return false;
}
 ...
 */
export function classifyHeroHandOnBoard(
  hero: [string, string],
  board: string[],
  turnType?: TurnType
): HeroHandClass {
  ...
  const pairQuality = classifyPairQuality(hero, board);

  // --- MADE HANDS (flush, straight) ---
  if (hasFlushMade(hero, board)) return "monster";
  if (hasStraightMade(hero, board)) return "monster";

  // --- QUADS ---
  for (const [, count] of allRanks.entries()) {
    if (count === 4) return "monster";
  }
  ...
  if (pairRanks.length >= 2) {
    ...
    if (heroPair && boardPairs.length >= 1 && !boardPairs.includes(pairRank!)) {
      const heroVal = valueOfRank(pairRank!);
      const topBoard = Math.max(...boardRanks.map(valueOfRank));
      if (heroVal > topBoard) {
        return "strong_value";
      }
      return "medium";
  ...
  if (heroPairRanksOnBoard.length >= 1) {
    const topBoard = Math.max(...boardRanks.map(valueOfRank));
    const heroPairVal = Math.max(...heroPairRanksOnBoard.map(valueOfRank));
    ...
    if (heroPairVal === topBoard) return "strong_value";
 ...
 */
export function getPairQuality(hero: [string, string], board: string[]): PairQuality {
  return classifyPairQuality(hero, board);
}
