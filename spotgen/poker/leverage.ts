...
export type LeverageProfile = "low" | "medium" | "high";

/**
 * BETTING MODE
 *
 * Overbets are NOT just bigger bets. They are a different strategic regime:
 *
 * | Aspect          | Standard bet      | Overbet           |
 * |-----------------|-------------------|-------------------|
 * | Goal            | Value / pressure  | Range collapse    |
 * | Target          | Medium-strength   | Bluff-catchers    |
 * | Frequency       | Common            | Rare              |
 * | Hand requirement| Good              | Very polarized    |
 * | Tree shape      | Dense             | Sparse            |
 * | Solver behavior | Mixes             | Extremes          |
 *
 * An overbet means: "Only nuts + chosen bluffs exist"
 */
export type BettingMode = "standard" | "overbet";
 ...
 */
export const BET_SIZE_SETS: Record<LeverageProfile, Partial<Record<BettingMode, [number, number]>>> = {
  low: {
    standard: [25, 33],
 ...
 */
export function getSizeSet(leverage: LeverageProfile, mode: BettingMode): [number, number] | null {
  const set = BET_SIZE_SETS[leverage]?.[mode];
 ...
 */
export function isValidLeverageMode(leverage: LeverageProfile, mode: BettingMode): boolean {
  return BET_SIZE_SETS[leverage]?.[mode] !== undefined;
}

/**
 * NODE INTENT from BETTING MODE
 *
 * NodeIntent must be derived from bettingMode, NOT heroClass.
 * This ensures frequencies align with the actual option semantics.
 *
 * - overbet → pressure (range collapse, targeting bluffcatchers)
 * - standard + high leverage → value or semi_bluff depending on hand
 * - standard + medium/low leverage → value
 */
export type NodeIntentKey = "value" | "pressure" | "semi_bluff" | "bluffcatch";

export function inferNodeIntentFromMode(
  mode: BettingMode,
 ...
 */
function hasNutStraight(hero: [string, string], board: string[]): boolean {
  const allCards = [...hero, ...board];
  const allValues = new Set(allCards.map(c => valueOfRank(rankOf(c))));
...
function checkHasStraight(values: Set<number>): boolean {
...
function getHighestStraightTop(values: Set<number>): number {
...
function getStraightCards(top: number): number[] {
  if (top === 5) {
    // Wheel
    return [14, 2, 3, 4, 5];
  }
 ...
 */
function hasNutFlush(hero: [string, string], board: string[]): boolean {
  const allCards = [...hero, ...board];

  // Find if we have a flush
  const suitCounts = new Map<string, string[]>();
  ...
  for (const [suit, cards] of suitCounts.entries()) {
    if (cards.length >= 5) {
      ...
      const heroSuitCards = hero.filter(c => suitOf(c) === suit);
      const heroHasAce = heroSuitCards.some(c => rankOf(c) === "A");
      ...
      const boardHasAce = board.some(c => suitOf(c) === suit && rankOf(c) === "A");
      if (boardHasAce) {
        const heroHasKing = heroSuitCards.some(c => rankOf(c) === "K");
        if (heroHasKing) return true;
      }
    }
  }
 ...
 */
function isDynamicBoard(board: string[]): boolean {
  const values = board.map(c => valueOfRank(rankOf(c)));
 ...
 */
export function computeLeverageProfile(args: {
  ...
  board: string[];
  hero: [string, string];
  pairQuality?: string; // Optional - from classifyPairQuality
}): LeverageProfile {
 ...
 */
export function hasStrongBlockers(hero: [string, string], board: string[]): boolean {
  const heroRanks = hero.map(c => rankOf(c));
  const heroValues = hero.map(c => valueOfRank(rankOf(c)));
  const boardValues = board.map(c => valueOfRank(rankOf(c)));
 ...
 */
export function computeSPR(effectiveStack: number, pot: number): number {
 ...
 */
export function inferBettingMode(args: {
  leverage: LeverageProfile;
  heroClass: HeroHandClass;
  handIntent: HandIntent;
  turnType: TurnType;
  ...
  pot: number;
  feats?: { hasStraight?: boolean; isWheelStraight?: boolean }; // Optional features
}): BettingMode {
 ...
 */
export function shouldBeAllIn(betAmount: number, remainingStack: number, pot: number): boolean {
