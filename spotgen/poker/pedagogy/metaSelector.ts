...
import type { PedagogyMeta, Street } from "./types";

/**
 * The anchor action (best action) affects meta selection.
 * "check" means the spot is about trapping/inducing.
 * "small"/"large"/"overbet" means the spot is about betting.
 */
export type AnchorAction = "check" | "small" | "large" | "overbet";

/**
 * Criteria for trap/slowplay eligibility.
 * These are STRICT gates — trap/slowplay must be RARE.
 *
 * TRAP REQUIREMENTS (ALL must be true):
 * 1. Anchor is check
 * 2. Street is RIVER only (turn traps are rare and require villain aggression)
 * 3. Polarity is polarized (trapping is a polar strategy)
 * 4. Nut advantage exists (trapping without nuts = pot-control)
 * 5. Betting is allowed (forced check ≠ trap)
 */
interface TrapEligibility {
  eligible: boolean;
  reason: string;
}

function checkTrapEligibility(
...
export function selectMeta(
  ...
  bettingContext: BettingContext,
  street: Street,
  anchor: AnchorAction = "small" // Default to betting for backwards compatibility
): PedagogyMeta {
...
function getMadeValueMeta(
  polarity: Polarity,
  street: Street,
  anchor: AnchorAction,
  trapCheck: TrapEligibility
): PedagogyMeta {
...
function getThinValueMeta(polarity: Polarity, street: Street, anchor: AnchorAction): PedagogyMeta {
  const streetName = streetToName(street);
...
function getComboDrawMeta(polarity: Polarity, street: Street): PedagogyMeta {
  const streetName = streetToName(street);
...
function getDrawMeta(polarity: Polarity, street: Street): PedagogyMeta {
  const streetName = streetToName(street);
...
function getPureBluffMeta(polarity: Polarity, street: Street): PedagogyMeta {
  const streetName = streetToName(street);
...
function getGiveUpMeta(polarity: Polarity, street: Street): PedagogyMeta {
  const streetName = streetToName(street);
...
function getDefaultMeta(polarity: Polarity, street: Street): PedagogyMeta {
  const streetName = streetToName(street);
...
function streetToName(street: Street): string {
  switch (street) {
    case "f":
      return "flop";
    case "t":
 ...
 */
export function filterConcepts(concepts: string[]): string[] {
  return concepts.filter((c) =>
    (ALLOWED_CONCEPTS as readonly string[]).includes(c)
  );
}
