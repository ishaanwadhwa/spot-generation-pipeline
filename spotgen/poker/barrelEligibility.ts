...
import { valueOfRank } from "./ranks";

export interface BarrelEligibility {
  canBarrelLarge: boolean;   // 75%+
  canBarrelSmall: boolean;   // 33-50%
  canCheck: boolean;         // always true, but explicit
  reason: string;            // why allowed/disallowed
}
 ...
 */
export function checkBarrelEligibility(
  heroClass: HeroHandClass,
  feats: HandFeatures,
  turnType: TurnType,
  boardRanks: number[], // valueOfRank for each board card
): BarrelEligibility {
  // Default: can check, can't barrel
  const base: BarrelEligibility = {
  ...
  if (feats.hasPair && feats.pairRank && turnType === "overcard_turn") {
    const pairVal = valueOfRank(feats.pairRank);
 ...
 */
export function isEligibleForBarrelSpot(
  heroClass: HeroHandClass,
  feats: HandFeatures,
  turnType: TurnType,
  boardRanks: number[],
