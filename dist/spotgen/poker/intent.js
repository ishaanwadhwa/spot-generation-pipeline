...
exports.classifyHandIntentWithContext = classifyHandIntentWithContext;
exports.classifyHandIntent = classifyHandIntent;
exports.inferNodeIntent = inferNodeIntent;
/**
 ...
 */
function classifyHandIntentWithContext(heroClass, feats, pairQuality, turnType) {
    ...
    if (feats.comboDraw || feats.hasPairPlusDraw)
        return "combo_draw";
    ...
    const dangerousTurns = ["straight_completer", "paired_turn", "flush_completer", "overcard_turn"];
    ...
    // Pure draws
    if (feats.hasFlushDraw || feats.straightDraw !== "none")
        return "draw";
    // Medium/weak without pair features
    if (heroClass === "medium" || heroClass === "weak")
        return "thin_value";
    return "pure_bluff";
}
/**
 ...
 */
function classifyHandIntent(heroClass, feats) {
    if (heroClass === "monster" || heroClass === "strong_value")
        return "made_value";
    if (feats.comboDraw)
        return "combo_draw";
...
function inferNodeIntent(spot) {
    ...
    if (concepts.includes("value-max"))
        return "value";
    if (concepts.includes("value-bet"))
        return "value";
    if (concepts.includes("protection-bet"))
        return "value";
    if (concepts.includes("trap"))
        return "value"; // Trapping is still a value play
    if (concepts.includes("slowplay"))
        return "value";
    if (concepts.includes("high-equity-combo-draw"))
        return "semi_bluff";
    if (concepts.includes("semi-bluff"))
        return "semi_bluff";
    if (concepts.includes("pot-control"))
        return "bluffcatch"; // Pot control = showdown value
    if (concepts.includes("showdown-value"))
        return "bluffcatch";
