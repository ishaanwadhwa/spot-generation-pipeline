...
exports.LINE_PATTERNS = void 0;
exports.selectLinePattern = selectLinePattern;
exports.buildPreflopHistory = buildPreflopHistory;
exports.calculatePotGeometry = calculatePotGeometry;
exports.calculateEffectiveStack = calculateEffectiveStack;
exports.appendPostflopHistory = appendPostflopHistory;
exports.buildPostflopLine = buildPostflopLine;
// ═══════════════════════════════════════════════════════════════════════════
// CONSTANTS
// ═══════════════════════════════════════════════════════════════════════════
 ...
 */
function selectLinePattern(rng) {
    const totalWeight = exports.LINE_PATTERNS.reduce((sum, p) => sum + p.weight, 0);
    let roll = rng() * totalWeight;
    for (const pattern of exports.LINE_PATTERNS) {
        roll -= pattern.weight;
        if (roll <= 0)
            return pattern;
    }
    return exports.LINE_PATTERNS[0]; // Fallback to standard
}
// ═══════════════════════════════════════════════════════════════════════════
 ...
 * - Caller calls 2.5bb
 */
function buildPreflopHistory(heroPos, villainPos, heroIsOpener) {
    const hist = [];
 ...
 */
function calculatePotGeometry(street, flopBetPct, turnBetPct = 0) {
 ...
 */
function calculateEffectiveStack(flopBetAmount, turnBetAmount) {
 ...
 */
function appendStreetAction(hist, action, oopPos, ipPos, betPct, betAmount) {
    switch (action) {
        case "ip_cbet":
 ...
 */
function appendPostflopHistory(hist, street, heroIsIP, heroPos, villainPos, flopBetPct, flopBetAmount, turnBetPct, turnBetAmount, pattern = exports.LINE_PATTERNS[0]) {
 ...
 */
function buildPostflopLine(input) {
    ...
    const { pot, flopBetAmount, turnBetAmount } = calculatePotGeometryWithPattern(street, flopBetPct, turnBetPct, pattern);
    ...
    const hist = buildPreflopHistory(heroPosition, villainPosition, heroIsOpener);
    // Step 6: Append postflop history with pattern
    appendPostflopHistory(hist, street, heroIsIP, heroPosition, villainPosition, flopBetPct, flopBetAmount, turnBetPct, turnBetAmount, pattern);
    return {
        hist,
        pot,
        effectiveStack,
 ...
 */
function calculatePotGeometryWithPattern(street, flopBetPct, turnBetPct, pattern) {
    // Flop: pot is just the preflop pot
    if (street === "f") {
        return { pot: PREFLOP_POT, flopBetAmount: 0, turnBetAmount: 0 };
    }
 ...
 */
function calculateEffectiveStackWithPattern(flopBetAmount, turnBetAmount, pattern) {
