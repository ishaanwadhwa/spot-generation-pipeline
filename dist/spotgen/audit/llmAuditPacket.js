...
exports.buildLLMAuditPacket = buildLLMAuditPacket;
exports.formatLLMAuditPacket = formatLLMAuditPacket;
...
function formatOptions(opts) {
    return opts.map((o, i) => {
        if (o[0] === "x")
            return `[${i}] Check`;
        if (o[0] === "b")
            return `[${i}] Bet ${o[1]}% (${o[2].toFixed(2)}bb)`;
        if (o[0] === "a")
            return `[${i}] All-in`;
        return `[${i}] ${o[0]}`;
    });
 ...
 */
function getRelevantTheory(repoRoot, street, handIntent, turnType) {
    const snippets = [];
    try {
        // Load core engine principles
        const coreEngine = path_1.default.join(repoRoot, "theory/core/CORE_HYBRID_ENGINE.md");
        if (fs_1.default.existsSync(coreEngine)) {
            const content = fs_1.default.readFileSync(coreEngine, "utf8");
        ...
        const postflopCore = path_1.default.join(repoRoot, "theory/postflop/explanations/postflop_core.md");
        if (fs_1.default.existsSync(postflopCore)) {
            const content = fs_1.default.readFileSync(postflopCore, "utf8");
        ...
        const matrixDoc = street === "t"
            ? path_1.default.join(repoRoot, "theory/postflop/examples/solver_truth_docs/turn_matrix.MD")
            : street === "r"
                ? path_1.default.join(repoRoot, "theory/postflop/examples/solver_truth_docs/river_matrix.MD")
                : path_1.default.join(repoRoot, "theory/postflop/examples/solver_truth_docs/flop_matrix.MD");
        if (fs_1.default.existsSync(matrixDoc)) {
            const content = fs_1.default.readFileSync(matrixDoc, "utf8");
            // Extract turnType-related content
            const turnTypeKeywords = {
 ...
 */
function buildLLMAuditPacket(repoRoot, spot) {
    ...
    const flopClass = spot.tags.find(t => ["dry_Axx_highcard", "dry_Kxx_Qxx", "low_disconnected", "medium_connected", "monotone", "paired"].includes(t)) || "unknown";
    // Theory context
 ...
 */
function formatLLMAuditPacket(packet) {
    const lines = [
        "═══════════════════════════════════════════════════════════════════════════",
